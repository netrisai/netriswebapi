...
func (c *Client) Update(id int, link *LinkU) (reply http.HTTPReply, err error) {
	js, err := json.Marshal(link)
	if err != nil {
		return http.HTTPReply{}, fmt.Errorf("{UpdateLink} %s", err)
	}
	address := c.client.URL.String() + v2address.Links + "/" + strconv.Itoa(id)
	reply, err = c.client.Put(address, js)
	if err != nil {
		return reply, fmt.Errorf("{UpdateLink} %s", err)
	}
