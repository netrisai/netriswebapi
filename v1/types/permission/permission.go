...
func parse(APIResult *http.APIResponse) ([]*PermissionGroup, error) {
	var sites []*PermissionGroup
...
func (c *Client) Get() ([]*PermissionGroup, error) {
	address := c.client.URL.String() + v1address.PermissionGroups
	APIResult, err := c.client.Get(address)
	if err != nil {
		return nil, fmt.Errorf("{GetPermissionGroups} %s", err)
	}
...
func (c *Client) Add(pgroup *PermissionGroupAdd) (reply http.HTTPReply, err error) {
...
func (c *Client) Update(pgroup *PermissionGroupAdd) (reply http.HTTPReply, err error) {
	js, err := json.Marshal(pgroup)
	if err != nil {
		return reply, err
	}
